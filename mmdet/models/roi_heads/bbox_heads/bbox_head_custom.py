...
class BBoxHeadCustom(BaseModule):
    ...
    def __init__(self,
                 ...
                 alpha_predictor_cfg=dict(type='Linear'),
                 loss_cls=dict(
        ...
        # add linear layer for alpha regression
        if self.with_alpha:
            out_dim_alpha = 1 * self.num_classes
            self.fc_alpha = build_linear_layer(
                self.alpha_predictor_cfg,
                in_features=in_channels,
                out_features=out_dim_alpha)
        ...
        if init_cfg is None:
            ...
            if self.with_alpha:
                self.init_cfg += [
                    dict(
                        type="Normal", std=0.001, override=dict(name="fc_alpha"))
                ]
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        """
        # TODO : add calclutation for alpha and returns
